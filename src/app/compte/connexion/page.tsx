import { sendClientLoginCode, verifyClientLoginCode } from "@/lib/actions/clientAuth";

interface ConnexionPageProps {
  searchParams: { etape?: string; email?: string; suite?: string; erreur?: string };
}

export default function ClientConnexionPage({ searchParams }: ConnexionPageProps) {
  const redirectTo = searchParams.suite || "/compte/tableau-de-bord";
  ...
  return (
    <div className="mx-auto max-w-sm px-4 py-16">
      ...
      <p className="mt-2 text-sm text-ink/60">
        Pas de mot de passe : un code à usage unique vous est envoyé par e-mail. Le
        compte est créé automatiquement à la première connexion.
      </p>
      ...
      {!isCodeStep ? (
        <form action={sendClientLoginCode} className="mt-6 space-y-4">
        ...
        <form action={verifyClientLoginCode} className="mt-6 space-y-4">
      ...
      <p className="mt-8 text-center text-sm text-ink/50">
        Un salon ?{" "}
        <a href="/pro" className="underline">
